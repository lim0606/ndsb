...
void DataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    timer.Start();
    // Do shuffle!! // jhlim
    if (this->layer_param_.data_param().shuffle()) {
      if (dataset_->size() > 0) {
        unsigned int skip = caffe_rng_rand() % (batch_size * 10); //caffe_rng_rand() % dataset_->size(); 
        ...
        LOG(ERROR) << "Something wrong with data shuffle. You may use LevelDB for your application. DataLayer with LevelDB currently does not have functionality of shuffle. Sorry. So, just read file normally."; 
      }
    }
    ...
    if (datum.encoded()) {
      ...
      //this->data_transformer_.Transform(datum, &(this->transformed_data_), tmp_, item_id); 
    }
    if (this->output_labels_) {
    ...
    if (!(this->layer_param_.data_param().shuffle()) || dataset_->size() <= 0) {
      // go to the next iter
      ++iter_;
      if (iter_ == dataset_->end()) {
        iter_ = dataset_->begin();
