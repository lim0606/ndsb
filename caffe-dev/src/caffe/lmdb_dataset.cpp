...
bool LmdbDataset<K, V, KCoder, VCoder>::open(const string& filename,
  ...
  // jhlim
  MDB_stat stat;
  retval = mdb_env_stat(env_, &stat);
  //LOG(INFO) << "what the fuck!!";
  if (MDB_SUCCESS != retval) {
    LOG(ERROR) << "mdb_env_stat failed " << mdb_strerror(retval);
    return false; 
  }
  ms_entries_ = stat.ms_entries;
...
template <typename K, typename V, typename KCoder, typename VCoder>
inline int LmdbDataset<K, V, KCoder, VCoder>::size() {
  return ms_entries_;
}
