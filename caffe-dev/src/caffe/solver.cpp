...
//    - step2: return base_lr * exp(alpha * ceil(iter / step))
...
Dtype SGDSolver<Dtype>::GetLearningRate() {
  ...
  } else if (lr_policy == "step2") {
    int current_step = std::ceil((float)(this->iter_) / (float)(this->param_.stepsize()));
    rate = this->param_.base_lr() *
        exp(this->param_.alpha() * current_step);
    //LOG(INFO) << "exp(1) = " << exp(1.) << ", rate = " << rate << ", current_step = " << current_step;
  } else {
